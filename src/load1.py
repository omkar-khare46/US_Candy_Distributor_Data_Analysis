...
load_dotenv()  # reads ../.env if the notebook lives in notebooks/
...
def create_schema() -> None:
    ...
    # statement .sql script needs statement-by-statement execution.
    # (execute(..., multi=True) is NOT used here: the C-extension cursor
    # that mysql-connector-python uses by default doesn't support it —
    # only the pure-Python cursor does, and forcing that adds its own
    # complications. Splitting ourselves is more portable across versions.)
    ...
    # Strip full-line comments, then split on ';'. Safe here because the
    # schema has no semicolons inside string literals or inline comments.
